...
per-clip quality metrics, and curation-filtered dataset export.
    ...
    uv run data/pipeline.py retarget <clip_name>          debug one clip
    uv run data/pipeline.py retarget --all --run v1.5-slug
    uv run data/pipeline.py metrics  [--run R] [--diff OTHER] [--emotions]
                                     [--dynamics sample|all] [--summary]
    uv run data/pipeline.py export   [--run R] [--min-frames 30]
                                     [--val-frac 0.1] [--include-unreviewed]
    uv run data/pipeline.py all --run v1.5-slug   retarget + metrics + export

Human curation (GIF review app, A/B panel, batch verdicts, contact
sheets) is optional and lives in data/scripts/curate.py; its verdicts
persist in data/lamp_data/curation.csv and gate the export stage.

RETARGET.  data/cozmo_data/animations/npz/<clip>.npz
    ->  data/lamp_data/npz/<run>/<clip>.npz
Every full-corpus invocation writes into its own run directory under
data/lamp_data/npz/, named by --run (default: unix timestamp). A run.json
...
METRICS.  Writes data/lamp_data/metrics/<run>.csv (git-tracked: the
...
data/lamp_data/dataset/lamp_dataset_<run>.npz + manifest_<run>.json.
"""
...
OUT = ROOT / "data" / "lamp_data"
NPZ_ROOT = OUT / "npz"           # one subdirectory per retarget run
...
# ---------------------------------------------------------------------------
# Run directories: data/lamp_data/npz/<run>/, run.json marks completeness
# ---------------------------------------------------------------------------
...
def latest_run():
    ...
    if not runs:
        sys.exit(f"no complete runs under {NPZ_ROOT} "
                 f"(run: uv run data/pipeline.py retarget --all --run <name>)")
    return runs[-1][1]
...
def write_report(stems, metrics, dyn, picks, labels, lamp, out_dir, run):
    ...
    lines = [
        ...
        "",
        f"Run **{run}**, generated by `data/pipeline.py retarget --all`. A fresh run:",
        "",
        "```",
        "uv run data/pipeline.py retarget --all --run <new-name>",
        "```",
        "",
        ...
        "docstring of data/pipeline.py for the full channel table and "
    ...
    lines += [
        "",
        f"## Output format (data/lamp_data/npz/{run}/<clip>.npz)",
        "",
        "| key | shape | meaning |",
        "|---|---|---|",
    ...
    lines += [
        "",
        "## Previews (data/lamp_data/preview/)",
        "",
...
def load_flags(run):
    ...
    if not path.exists():
        sys.exit(f"no metrics for run '{run}' -- run "
                 f"`uv run data/scripts/eval_retarget.py --run {run}` first")
    with open(path, newline="") as fh:
...
def main():
    ...
    pr.add_argument("--run", default=None,
                    help="--all: run name -> data/lamp_data/npz/<run>/ "
