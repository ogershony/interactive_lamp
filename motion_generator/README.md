# motion_generator: conditional flow matching for expressive lamp motion

Generates whole variable-length lamp clips — 5 joint trajectories + LED
brightness + LED color, 9 channels at 30 Hz — conditioned on an 11-d
affect vector. Trained on the curated Cozmo-retarget dataset
(`data/dataset/lamp_dataset_v1.5.npz`, 734 train / 78 val
clips, 86k frames). See `lamp_plan.md` §7 for the design rationale.
...
`clip_offsets`, `emotions (n,11)` (stored sum-to-1), `split`, plus the
sibling `manifest_*.json` for clip names.

**Conditioning convention (must match at prompt time):** the 11-d affect
...
python motion_generator/train.py --device cuda \
    --npz path/to/lamp_dataset_v1.5.npz --out-dir runs/fm-v1
```
...
`runs/fm-v1/ckpt_best.pt` (20 MB) is git-tracked — the one exception
...
# -> motion_generator/outputs/joy07+surprise03_0.npz   (add --gif for a render)
```

`sample.py` is the full-control version:

```bash
uv run motion_generator/sample.py --ckpt motion_generator/runs/fm-v1/ckpt_best.pt \
...
```bash
uv run motion_generator/evaluate.py --ckpt ... --out eval_report.md
```
