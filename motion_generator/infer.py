...
"""One-shot inference: affect combo in, lamp motion clip out.
    ...
    uv run motion_generator/infer.py joy
    uv run motion_generator/infer.py "joy=0.7,surprise=0.3" --seconds 3
    uv run motion_generator/infer.py "anger=0.6,frustration=0.4" --n 4 --gif
...
- the checkpoint defaults to runs/fm-v1/ckpt_best.pt next to this file
- output goes to motion_generator/outputs/ (gitignored), named
...
DEFAULT_CKPT = HERE / "runs" / "fm-v1" / "ckpt_best.pt"
