...
2. PER-AFFECT FEATURE AGREEMENT: Spearman rank correlation across the 11
...
# 6-class grouping of the 11 affects (label-noise-robust eval classes;
# adjust to the EMRO grouping for the writeup if it differs)
GROUPS = {
    "joy":      ["joy"],
    "interest": ["interest", "understanding"],
...
assert len(G_OF_E) == len(EMOTIONS)


def group_label(label16):
    """Unit-L2 affect vector -> dominant 6-class index."""
...
def main():
    ...
    p.add_argument("--npz",
                   default="data/dataset/lamp_dataset_v1.5.npz")
    ...
    log(f"# motion_generator evaluation: {args.ckpt}")
    ...
    for a, b in (("joy", "sorrow"), ("interest", "boredom")):
        va = np.zeros(len(EMOTIONS))
        va[EMOTIONS.index(a)] = 1
        vb = np.zeros(len(EMOTIONS))
    ...
    lab = unit(np.eye(len(EMOTIONS))[EMOTIONS.index("joy")])
    ...
    lab = unit(np.eye(len(EMOTIONS))[EMOTIONS.index("surprise")])
