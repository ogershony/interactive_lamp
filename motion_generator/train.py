...
Train the conditional flow-matching style model.
    ...
    uv run motion_generator/train.py --device cpu --steps 200 --overfit 10   # G0 smoke
    python motion_generator/train.py --device cuda                          # full run
...
from dataset import EMOTIONS, build
from model import EMA, Denoiser, count_params

DEFAULTS = dict(
    npz="data/dataset/lamp_dataset_v1.5.npz",
    out_dir="motion_generator/runs/fm-v1",
...
def main():
    ...
    # record the affect width in the checkpoint config so load_model can
    # rebuild the exact architecture even if the taxonomy changes again
    cfg["n_affect"] = len(EMOTIONS)
    model = Denoiser(d=cfg["d"], heads=cfg["heads"], ff=cfg["ff"],
                     layers=cfg["layers"], n_affect=cfg["n_affect"]).to(device)
